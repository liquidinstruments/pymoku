...
from . import _utils
from . import InvalidOperationException
from . import NotDeployedException
from . import StreamException
...
class InputInstrument(_instrument.MokuInstrument):
    """
        ...
        This class is intended to handle streams: checking status,
        receiving/parsing raw data, translating error messages and setting up
    ...
    """
        Expose all the get/set input instrument functions from the
        MokuInstrument class
    """
    def set_frontend(self, channel, fiftyr=True, atten=False, ac=False):
        ...
        :type atten: bool
        :param atten: Turn on 10x attenuation. Changes the dynamic range
            between 1Vpp and 10Vpp.
        ...
        return super(InputInstrument, self)._set_frontend(
            channel, fiftyr, atten, ac)

    def get_frontend(self, channel):
        """ Get the analog frontend configuration.

        :type channel: int; {1,2}
        :param channel: Channel for which the relay settings are being
            retrieved
    ...
    def _max_stream_rate(self, use_sd, filetype):
        """
        Returns the maximum rate at which the instrument can be streamed for
        the given streaming configuration
        """

        # These are checked on the client side too but we sanity-check here
        # as an invalid rate can hard-hang the Moku. These rates are
        # approximate and experimentally derived, should be updated as we
        # test and optimize things.
        # Logging rates depend on which storage medium, and the filetype as
        # well
        maxrates = None

        # The record length in the dataparser is in bits, convert to words;
        # that's the unit of the rate (more precisely, 32-bit samples is
        # the unit)
        record_length = math.ceil(
            dataparser.LIDataParser.record_length(self.binstr) / 32.0)
    ...
    def _estimate_logsize(self, ch1, ch2, duration, filetype):
        if filetype in ['bin', 'mat', 'npy']:
            ...
            record_length = math.ceil(
                dataparser.LIDataParser.record_length(self.binstr) / 8.0)
            sample_size_bytes = record_length * (ch1 + ch2)
            return (duration / self.timestep) * sample_size_bytes
        elif filetype == 'csv':
            # Assume that if the string includes [, it's expecting data arrays
            if '[' in self.fmtstr:
                # Assume no instrument provides more than 10 entries per record
                ch1, ch2 = [[-1] * 10] * 2
            else:
                ch1, ch2 = -1, -1
            # Dummy values chosen for maximum formatted length
            f = self.fmtstr.format(ch1=ch1, ch2=ch2, t=-1, T=-1, n=1, d=0.1)
    ...
    def _stream_start(self, start, duration, use_sd, ch1, ch2, filetype):
        ...
        If the duration is non-zero, the device must be in ROLL mode (via a
        call to :any:`set_xmode`). Also, the sample rate must be appropriate
        to the instrument and file type (as calculated by
        :any:`_max_stream_rates`)

        :raises InvalidOperationException: if the sample rate is too high for
        the selected filetype or if the device *x_mode* isn't set to *ROLL*
        when running non-'net' type streams.

        .. warning:: Start parameter not currently implemented, must be set
        to zero
        ...
        *File Types*
        ...
        - **net** -- Log to network, retrieve data with :any:`
                     _stream_receive_samples`
        """
        if (not ch1) and (not ch2):
            raise InvalidOperationException(
                "No channels were selected for logging")
        if duration < 0:
        ...
        # This breaks in the firmware due to a check for valid filename
        # for all session types
        fname = datetime.now().strftime(self.logname + "_%Y%m%d_%H%M%S")

        # Currently the data stream genesis is from the x_mode commit below,
        # meaning that delayed start doesn't work properly. Once this is
        # fixed in the FPGA/daemon, remove this check and the note in the
        # documentation above.
        if start:
            raise InvalidOperationException(
                "Logging start time parameter currently not supported")

        # Logging rates depend on which storage medium, and the filetype
        # as well
        if duration > 0:

            if self.x_mode != _instrument.ROLL:
                raise InvalidOperationException(
                    "Instrument must be in roll mode to perform data logging")

        if not all([len(s) for s in [self.binstr,
                                     self.procstr,
                                     self.fmtstr,
                                     self.hdrstr]]):
            raise InvalidOperationException(
                "Instrument currently doesn't support data logging")

        # We have to be in this mode anyway because of the above check, but
        # rewriting this register and committing is necessary in order to
        # reset the channel buffers on the device and flush them of old data.
        if duration > 0:
            self.x_mode = _instrument.ROLL
            self.commit()

        try:
            self._moku._stream_prep(ch1=self.ch1,
                                    ch2=self.ch2,
                                    start=start,
                                    end=start + duration,
                                    offset=0,
        ...
        # This may not actually exist as a file (e.g. if a 'net'
        # session was run)
    ...
    def _stream_stop(self):
        """ Stop a recording session previously started
            with :py:func:`datalogger_start`

        This function signals that the user no longer needs to know the status
        of the previous log, discarding that state. It must be called before
        a second log is started or else that start attempt will fail with a
        "busy" error.
    ...
    def _stream_status(self):
        """ Return the status of the most recent recording session to be
        started. This is still valid after the stream has stopped, in which
        case the status will reflect that it's safe to start a new session.

        Returns a tuple of state variables:

        - **status** -- Current datalogger state
        - **logged** -- Number of samples recorded so far. If more than one
                        channel is active, this is the sum of all points
                        across all channels.
        - **to start** -- Number of seconds until/since start. Time until
                          start is positive, a negative number indicates
                          that the record has started already.
        - **to end** -- Number of seconds until/since end.
        - **filename** -- Base filename of current log session
                          (without filetype)
        ...
        - **_STREAM_STATE_INVAL** -- An attempt was made to start a session
                                     with invalid parameters
        - **_STREAM_STATE_FSFULL** -- A session has terminated early due to
                                      the storage filling up
        - **_STREAM_STATE_OVERFLOW** -- A session has terminated early due to
                                        the sample rate being too high for the
                                        storage speed
        - **_STREAM_STATE_BUSY** -- An attempt was made to start a session
    ...
    def _stream_completed(self, status=None):
        """ Returns whether or not the datalogger is expecting to log any
            more data.

        If the log is completed then the results files are ready to be
        uploaded or simply read off the SD card. At most one subsequent
        :any:`get_stream_data` call will return without timeout.

        If the datalogger has entered an error state,
        a StreamException is raised.
        ...
        if status == _STREAM_STATE_NONE:
            raise StreamException("Attempted to check progress"
                                  " of non-existent streaming session.")

        return status not in [_STREAM_STATE_RUNNING, _STREAM_STATE_WAITING]

    def _stream_error(self, status=None):
        """ Checks the current datalogger session for errors.
        Alternatively, the status parameter returned by :any:
        `_stream_status` call can be translated to the associated exception
        (if any).
        ...
        if status in [_STREAM_STATE_NONE,
                      _STREAM_STATE_RUNNING,
                      _STREAM_STATE_WAITING,
                      _STREAM_STATE_STOPPED]:
            msg = None
            ...
            msg = "Tried to start a logging session while one was "
            "already running."
        else:
    ...
    def _stream_receive_samples(self, timeout):
        """
            ...
            :raises DataIntegrityException: If the network layer detects
                dropped data
        """
    ...
    def _streamsub_init(self, tag):
        """
            Initialises a ZMQ stream subscriber and data parser for current
            session.
        """
        ...
        # Zero offset from start time to first sample, valid for streams
        # but not so much for single frame transfers
        self._strparser = dataparser.LIDataParser(self.ch1,
                                                  self.ch2,
                                                  self.binstr,
                                                  self.procstr,
                                                  self.fmtstr,
    ...
    def _update_datalogger_params(self):
        # To be overwritten by child instruments to update local datalogger
        # parameters prior to starting any stream session
