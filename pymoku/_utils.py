...
from pymoku import InvalidParameterException
import sys
...
def str_to_val(valmap, key, param_description):
    """
        ...
        Helper function - enables mapping of client strings to bit/register
        values.
    """
    try:
        ...
        raise InvalidParameterException(
            "Invalid %s : \'%s\'. Expected %s." % (param_description,
                                                   key,
                                                   valmap.keys()))


def check_parameter_valid(
        check_type, v, allowed=None, desc="", units="", allow_none=False):
    ...
    if check_type == 'bool':
        ...
        try:
            ...
            if not isinstance(v, basestring):
                raise InvalidParameterException(
                    "Invalid parameter \'%s\': %s. Expected string." % (desc,
                                                                        v))
        except NameError:
        ...
        if not (v in allowed):
            raise InvalidParameterException(
                "Invalid parameter \'%s\': %s. Valid set %s %s." % (desc,
                                                                    v,
                                                                    allowed,
                                                                    units))
        ...
        elif isinstance(allowed, tuple) and not (v > allowed[0] and
                                                 v < allowed[1]):
        ...
        raise InvalidParameterException(
            "Invalid parameter 'check_type': %s. "
            "Expected ['bool','set','range']." % check_type)
