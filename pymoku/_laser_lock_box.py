...
from pymoku._iir_block import IIRBlock
...
class LaserLockBox(_CoreOscilloscope):
    def __init__(self):
        ...
        self.demod_sweep = SweepGenerator(
            self, reg_base=REGBASE_LLB_LO)
        self.scan_sweep = SweepGenerator(
            self, reg_base=REGBASE_LLB_SCAN)
        self.aux_sine_sweep = SweepGenerator(
            self, reg_base=REGBASE_LLB_AUX)
        self.iir_filter = IIRBlock(
            self, reg_base=REGBASE_LLB_IIR, num_stages=2,
            gain_frac_width=9, coeff_frac_width=30, use_mmap=False)
    ...
    def set_defaults(self):
        ...
        self.set_trigger('scan', 'rising', level=0, hysteresis=0.1,
    ...
    @needs_commit
    def _set_scale(self):
        # incorporate adc2 scaling if local oscillator source is set to
        # 'external'.
        self.lo_scale_factor = (
            1.0 if self.MuxLOSignal == 0 else
            self._adc_gains()[1] * 2**12 /
            (10.0 if self.get_frontend(2)[1] else 1.0))

        self._fast_scale = (
            self._adc_gains()[0]
            / self._dac_gains()[0]
            / 2**3 * self.lo_scale_factor
    ...
    def set_custom_filter(self, filt_coeffs):
        ...
        :param filt_coeffs: array containg Direct-Form 1 SOS filter
        ...
        Each 'a' coefficient must be a float in the range [-2.0, +2.0). 's'
        coefficients are multiplied into each 'b' coefficient before being
        sent to the device. These products (sN x b0.N, sN x b1.N, sN x b2.N)
        must also fall in the range [-2.0, +2.0). Internally, the 'a' and 'b'
        coefficients are represented as signed 32-bit fixed-point numbers,
        with 30 fractional bits.

        Filter coefficients can be computed using signal processing
        toolboxes in e.g. MATLAB or SciPy.
        ...
        _utils.check_parameter_valid(
            ...
            len(filt_coeffs),
            [2],
            desc='number of a/b filter array rows')
        _utils.check_parameter_valid(
            'set',
            len(filt_coeffs[0]),
            [6],
            desc='number of coefficients in first filter array row')
        _utils.check_parameter_valid(
            'set',
            len(filt_coeffs[1]),
            [6],
            desc='number of coefficients in second filter array row')
        ...
        for row in range(0, 2):
            for bcoef in range(1, 4):
                ...
                _utils.check_parameter_valid(
                    ...
                    [-2.0, 2.0],
                    desc=('product of b{}.{} and s{}'
                          .format(row + 1, bcoef, row + 1)),
                    units='linear scalar')
        filt_coeffs[0][0] = 1.0
        filt_coeffs[1][0] = 1.0
        ...
        for row in range(0, 2):
            for coef in range(0, 6):
                _utils.check_parameter_valid(
                    'range',
                    filt_coeffs[row][coef],
                    [-2.0, 2.0],
                    desc='coefficient value',
                    units='linear scalar')
    ...
    def set_output_range(self, ch, maximum, minimum):
        ...
        _utils.check_parameter_valid(
            'range', maximum, [-1.0, 1.0], desc='maximum', units='Volts')
        _utils.check_parameter_valid(
            'range', minimum, [-1.0, 1.0], desc='minimum', units='Volts')
        if minimum > maximum:
    ...
    def set_offsets(self, position, offset):
        ...
        if position == 'pid_input':
            self.fast_offset = (
                offset / (self._adc_gains()[0] * 2**12) / self.lo_scale_factor)
        elif position == 'out1':
            self.output_offset_ch1 = (
                offset / (self._adc_gains()[0] * 2**12) / self.lo_scale_factor)
        else:
            self.output_offset_ch2 = (
                offset / (self._adc_gains()[0] * 2**12) / self.lo_scale_factor)

    @needs_commit
    def set_pid_by_gain(self, pid_block, g=1, kp=1, ki=0, kd=0,
                        si=None, sd=None, enable=True):
        """
        ...
        :type ki: float;
            [0, 31.25e6] with a resolution of 31.25e6 / 2^24-1
            when pid_block = 1
            [0, 488.28e3] with a resolution of 488.28 / 2^24-1
            when pid_block = 2.
        :param ki: Integrator gain factor.

        :type kd: float; [0, 31.25e6] when pid_block = 1,
        [0, 488.28e3] when pid_block = 2
        ...
        :raises InvalidConfigurationException: if the configuration of
        PID gains is not possible.
        """

        _utils.check_parameter_valid(
            'set', pid_block, [1, 2], 'filter channel')
        _utils.check_parameter_valid(
            'range',
            g,
            [0, 2**16 - 1],
        ...
        if pid_block == 1:
            _utils.check_parameter_valid(
                ...
                ki,
                [0, 31.25e6],
                desc='integrator gain',
                units='linear scalar')
            _utils.check_parameter_valid(
                'range',
                kd, [0, 31.25e6],
                desc='differentiator gain',
                units='linear scalar')
        else:
            _utils.check_parameter_valid(
                'range',
        ...
        if si is not None:
            _utils.check_parameter_valid(
                ...
                si,
                [-1e3, 1e3],
                desc='integrator gain saturation',
                units='linear scalar')
        if sd is not None:
            _utils.check_parameter_valid(
                'range',
                sd,
                [-1e3, 1e3],
                desc='differentiator gain saturation',
                units='linear scalar')
    ...
    @needs_commit
    def set_pid_by_freq(self, pid_block, kp=1, i_xover=None, d_xover=None,
                        si=None, sd=None, enable=True):
        """
        ...
        if i_xover is not None:
            _utils.check_parameter_valid(
                'range',
                i_xover,
                [1e-3, 1e6],
                desc='integrator cross over'
                ' frequency',
                units='hertz')
        if d_xover is not None:
            _utils.check_parameter_valid(
                'range',
                d_xover,
                [1, 10e6],
                desc='differentiator cross'
                ' over frequency',
                units='hertz')
        if si is not None:
            _utils.check_parameter_valid(
    ...
    def set_pid_enables(self, pid_block, en=True):
        ...
        """
        _utils.check_parameter_valid(
            'set', pid_block, [1, 2], 'PID controller')
        _utils.check_parameter_valid(
    ...
    @needs_commit
    def set_local_oscillator(self, frequency=0.0, phase=0.0,
                             source='internal', pll_auto_acq=True):
        """
        Configure the local oscillator. The local oscillator can be generated
        internally with configurable frequency and phase offset,
        externally via ADC 2 or internally with a PLL phase locked to ADC 2
        plus an additional phase offset.
        ...
        _utils.check_parameter_valid(
            ...
            frequency,
            [0, 200e6],
            desc='local oscillator frequency',
            units='hertz')
        _utils.check_parameter_valid(
            'range',
            phase,
            [0, 360],
            desc='local oscillator phase offset',
            units='degrees')
        _utils.check_parameter_valid(
            'set',
            source,
            ['internal', 'external', 'external_pll'],
            'local oscillator source')
        _utils.check_parameter_valid(
            'set',
            pll_auto_acq,
            [True, False],
            'enable pll auto acquire')
    ...
    @needs_commit
    def set_aux_sine(self, amplitude=2.0, frequency=0.0, phase=0.0,
                     sync_to_lo=False, output='out2'):
        """
        ...
        :param sync_to_lo : True = enable phase synchronisation to local
            oscillator, False = use auxiliary frequency and phase values.
        ...
        _utils.check_parameter_valid(
            ...
            phase,
            [0, 360],
            desc='aux phase offset',
            units='degrees')
        _utils.check_parameter_valid(
            'set',
            sync_to_lo,
            [True, False],
            'sync phase to local oscillator')
        _utils.check_parameter_valid(
            'set',
            output,
            ['out1', 'out2', 'none'],
    ...
    @needs_commit
    def set_scan(self, amplitude=2.0, frequency=0.0, phase=0.0,
                 waveform='triangle', output='out1'):
        """
        ...
        self.scan_sweep.step = (
            frequency * (
                _LLB_FREQSCALE if waveform == _LLB_SCAN_SAWTOOTH
                else frequency * _LLB_FREQSCALE * 2))
        ...
        if output == _LLB_SCANSOURCE_DAC1:
            ...
            self.fast_scan_enable = False
            self.slow_scan_enable = False
            self.scan_amplitude = (  # default to out 1 scale
                (amplitude / 2.0) / self._dac_gains()[0] / 2.0**15)
    ...
    def _signal_source_volts_per_bit(self, source, scales, trigger=False):
        ...
        if ((not trigger and self.is_precision_mode())
                or (trigger and self.trig_precision)):
        ...
        if (source == _LLB_SOURCE_A):
            ...
            level = self._monitor_source_volts_per_bit(
                'scan', scales) / deci_gain
        elif (source == _LLB_SOURCE_IN1):
            level = self._monitor_source_volts_per_bit(
                'in1', scales) / deci_gain
        elif (source == _LLB_SOURCE_IN2):
            level = self._monitor_source_volts_per_bit(
                'in2', scales) / deci_gain
    ...
    def _monitor_source_volts_per_bit(self, source, scales):
        ...
        monitor_source_gains = {
            ...
            'scan': 2**4 * (scales['gain_dac2']
                            if self.slow_scan_enable
                            else scales['gain_dac1']),
            'lo': scales['gain_adc2'] / (10.0
                                         if scales['atten_ch2']
                                         else 1.0)
    ...
    def set_trigger(self, source, edge, level, minwidth=None,
                    maxwidth=None, hysteresis=10e-2, hf_reject=False,
                    mode='auto', trig_on_scan_rising=False):
        """
        ...
        self._set_trigger(source, edge, level, minwidth, maxwidth,
    ...
    def set_monitor(self, monitor_ch, source):
        ...
        """
        _utils.check_parameter_valid(
            'string', monitor_ch, desc="monitor channel")
        _utils.check_parameter_valid(
            'string', source, desc="monitor signal")
...
_llb_reg_hdl = {
    ...
    'fast_scan_enable':
        (REG_LLB_SCAN_CTRL, to_reg_unsigned(0, 1), from_reg_unsigned(0, 1)),

    'slow_scan_enable':
        (REG_LLB_SCAN_CTRL, to_reg_unsigned(1, 1), from_reg_unsigned(1, 1)),

    'lo_phase_offset':
        (REG_LLB_LO_PHASE_OFFSET,
            to_reg_unsigned(0, 28),
            from_reg_unsigned(0, 28)),

    'aux_phase_offset':
        (REG_LLB_AUX_PHASE_OFFSET,
            to_reg_unsigned(0, 28),
            from_reg_unsigned(0, 28)),

    'fast_offset':
        (REG_LLB_OFFSETS_FASTINPUT,
            to_reg_signed(0, 18, xform=lambda obj, x: x * 2**15),
            from_reg_signed(0, 18, xform=lambda obj, x: x / 2**15)),

    'output_offset_ch1':
        (REG_LLB_OFFSETS_FASTOUTPUT,
            to_reg_signed(0, 18, xform=lambda obj, x: x * (2**15)),
            from_reg_signed(0, 18, xform=lambda obj, x: x / (2**15))),

    'output_offset_ch2':
        (REG_LLB_OFFSETS_SLOWOUTPUT,
            to_reg_signed(0, 18, xform=lambda obj, x: x * (2**15)),
            from_reg_signed(0, 18, xform=lambda obj, x: x / (2**15))),

    'monitor_select0':
        (REG_LLB_MON_SEL, to_reg_unsigned(0, 4), from_reg_unsigned(0, 4)),

    'monitor_select1':
        (REG_LLB_MON_SEL, to_reg_unsigned(4, 4), from_reg_unsigned(4, 4)),

    'input_gain_select':
        (REG_LLB_GAINS_INPUT, to_reg_unsigned(0, 2), from_reg_unsigned(0, 2)),

    'MuxLOPhase':
        (REG_LLB_LO_CTRL, to_reg_unsigned(0, 1), from_reg_unsigned(0, 1)),

    'MuxLOSignal':
        (REG_LLB_LO_CTRL, to_reg_unsigned(1, 1), from_reg_unsigned(1, 1)),

    'MuxAuxPhase':
        (REG_LLB_AUX_CTRL, to_reg_unsigned(0, 1), from_reg_unsigned(0, 1)),

    'trig_aux':
        (REG_LLB_MON_SEL, to_reg_unsigned(8, 1), from_reg_unsigned(8, 1)),

    'cond_trig':
        (REG_LLB_MON_SEL, to_reg_unsigned(9, 1), from_reg_unsigned(9, 1)),

    'cliprange_lower_ch1':
        (REG_LLB_CLIP_FAST,
            to_reg_signed(0, 16, xform=lambda obj, x: x * 2**15 - 1),
            from_reg_signed(0, 16, xform=lambda obj, x: x / (2**14 - 1))),

    'cliprange_upper_ch1':
        (REG_LLB_CLIP_FAST,
            to_reg_signed(16, 16, xform=lambda obj, x: x * 2**15 - 1),
    ...
    'fast_aux_enable':
        (REG_LLB_AUX_CTRL, to_reg_bool(1), from_reg_bool(1)),

    'slow_aux_enable':
        (REG_LLB_AUX_CTRL, to_reg_bool(2), from_reg_bool(2)),

    'fast_channel_en':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(2), from_reg_bool(2)),

    'slow_channel_en':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(3), from_reg_bool(3)),

    'out1_en':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(4), from_reg_bool(4)),

    'out2_en':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(5), from_reg_bool(5)),

    'input1_light':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(6), from_reg_bool(6)),

    'input2_light':
        (REG_LLB_ENABLES_LIGHTS, to_reg_bool(7), from_reg_bool(7))
