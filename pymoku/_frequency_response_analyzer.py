...
class FrequencyResponseAnalyzer(_frame_instrument.FrameBasedInstrument):
    ...
    def set_sweep(self, f_start=100, f_end=120e6, sweep_points=512,
        ...
        _utils.check_parameter_valid('range', sweep_points, [32, 512],
                                     'sweep points')
        _utils.check_parameter_valid('bool', sweep_log,
        ...
        self.sweep_freq_delta = self._calculate_sweep_delta(f_start,
                                                            f_end,
                                                            sweep_points,
                                                            sweep_log)
    ...
    def start_sweep(self, single=False):
        ...
        """
        _utils.check_parameter_valid('bool', single,
    ...
    def stop_sweep(self):
        ...
        This will stop new data frames from being received, so ensure you
        implement a timeout on :any:
        `get_data<pymoku.instruments.FrequencyResponseAnalyzer.get_data>`
        calls.
        """
    ...
    def enable_amplitude(self, ch, en=True):
        ...
        """
        _utils.check_parameter_valid('set', ch, [1, 2], 'output channel')
        _utils.check_parameter_valid('set', en, [True, False],
                                     'enable amplitude')
    ...
    def set_ch_phase(self, ch, phase=0.0):
        ...
        _utils.check_parameter_valid('range', phase, [0.0, 360.0],
                                     'phase', 'degrees')
        if ch == 1:
    ...
    def get_data(self, timeout=None, wait=True):
        ...
        In the FrequencyResponseAnalyzer this is an alias for
        ``get_realtime_data`` as the data is never downsampled.
        """
        return super(FrequencyResponseAnalyzer,
                     self).get_realtime_data(timeout, wait)
    ...
    'sweep_freq_min':
        ((REG_FRA_SWEEP_FREQ_MIN_H, REG_FRA_SWEEP_FREQ_MIN_L),
            to_reg_unsigned(0, 48,
                            xform=lambda obj, f: f * _FRA_FREQ_SCALE),
    ...
    'sweep_offset_ch2':
        (REG_FRA_SWEEP_OFF_MULT,
            ...
            from_reg_signed(16, 16,
                            xform=lambda obj, a: a * obj._dac_gains()[1])),
    'settling_cycles':
        (REG_FRA_SETTLE_CYCLES,
            to_reg_unsigned(0, 32),
            from_reg_unsigned(0, 32)),
    'averaging_cycles':
        (REG_FRA_AVERAGE_CYCLES,
            to_reg_unsigned(0, 32),
            from_reg_unsigned(0, 32)),
    'ch1_harmonic_mult':
        (REG_FRA_HARMONIC_MULT_CH1,
            to_reg_unsigned(0, 4),
            from_reg_unsigned(0, 4)),
    'ch2_harmonic_mult':
        (REG_FRA_HARMONIC_MULT_CH2,
            to_reg_unsigned(0, 4),
            from_reg_unsigned(0, 4)),
    'ch1_meas_phase':
        ((REG_FRA_PHASE_OFF_CH1_MSB, REG_FRA_PHASE_OFF_CH1_LSB),
            to_reg_unsigned(0, 64),
            from_reg_unsigned(0, 64)),
    'ch2_meas_phase':
        ((REG_FRA_PHASE_OFF_CH2_MSB, REG_FRA_PHASE_OFF_CH2_LSB),
            to_reg_unsigned(0, 64),
            from_reg_unsigned(0, 64))
}
