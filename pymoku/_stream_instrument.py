...
from pymoku import _instrument
from pymoku import _utils
from pymoku._instrument import UncommittedSettings
from pymoku._instrument import ValueOutOfRangeException
from pymoku._instrument import InvalidOperationException
from pymoku._instrument import NoDataException
from pymoku._instrument import NotDeployedException
from pymoku._instrument import MPNotMounted

log = logging.getLogger(__name__)


class StreamBasedInstrument(_input_instrument.InputInstrument,
    ...
    def __init__(self):
        ...
        # Flag to indicate if there is no more stream data to get for
        # last session
        self._no_data = True
    ...
    def start_stream_data(self, duration=10, ch1=True, ch2=True):
        ...
        All outstanding settings must have been committed before starting the
        stream. This will always be true if *pymoku.autocommit=True*, the
        default.
        ...
        _utils.check_parameter_valid('float', duration,
                                     desc='stream duration', units='sec')

        if self.check_uncommitted_state():
            raise UncommittedSettings("Can't start a streaming session due"
                                      " to uncommitted device settings.")
        self._stream_start(start=0, duration=duration, ch1=ch1,
                           ch2=ch2, use_sd=False, filetype='net')
        self._no_data = False

    def stop_stream_data(self):
        """ Stops instrument data being streamed over the network.

        Should be called exactly once for every `start_stream_data` call,
        even if the streaming session stopped itself due to timeout. Calling
        this function not only causes the stream to stop, but also resets
        error and transfer state, ready to start a new streaming session.
        """
        self._no_data = True
        self._stream_stop()
    ...
    def get_stream_data(self, n=0, timeout=None):
        ...
        to_return = min([len(p) for c, p in
                         zip(active_channels, processed_samples) if c])
    ...
    def start_data_log(self, duration=10, ch1=True, ch2=True,
                       use_sd=True, filetype='csv'):
        """ Start logging instrument data to a file.

        Progress of the data log may be checked calling `progress_data_log`.

        All outstanding settings must have been committed before starting the
        data log. This will always be true if *pymoku.autocommit=True*,
        the default.

        .. note:: The Moku's internal filesystem is volatile and will be
            wiped when the Moku is turned off. If you want your data logs to
            persist either save to SD card or move them to a permanent
        ...
        :param ch2: Enable streaming on Channel 2
        :type use_sd: bool
        :param use_sd: Whether to log to the SD card, else the internal
            Moku filesystem.
        :type filetype: string
        :param filetype: Log file type, one of {'csv','bin','mat','npy'}
            for CSV, Binary, MATLAB or NPY (Numpy Data) respectively.
        ...
        _utils.check_parameter_valid(
            'float', duration, desc='log duration', units='sec')
        _utils.check_parameter_valid(
            'set', filetype, ['csv', 'mat', 'bin', 'npy'], 'log filetype')

        if self.check_uncommitted_state():
            raise UncommittedSettings("Can't start a logging session due to"
                                      " uncommitted device settings.")
        self._stream_start(start=0, duration=duration, ch1=ch1, ch2=ch2,
                           use_sd=use_sd, filetype=filetype)

    def stop_data_log(self):
        """ Stops the current instrument data logging session.

        This must be called exactly once for every `start_data_log` call,
        even if the log terminated itself due to timeout. Calling this
        function doesn't just stop the session (if it isn't already stopped),
        but also resets error and transfer state, ready to start a new logging
        session.
        """
        self._stream_stop()

    def progress_data_log(self):
        """ Estimates progress of a logging session started by a
            `start_data_log` call.

        :rtype: float
        :returns: [0.0-100.0] representing 0 - 100% completion of the current
                logging session.Note that 100% is only returned when the
                session has completed, the progress may pause at 99% for a
                time as internal buffers are flushed.
        :raises: StreamException: if an error occurred with the current
                logging session.
        """
    ...
    def data_log_filename(self):
        ...
        The base filename doesn't include the file extension as multiple
        files might be recorded simultaneously with different extensions.
    ...
    def upload_data_log(self):
        """ Load most recently recorded data file from the Moku to the local
            PC.
        ...
        if not target:
            raise InvalidOperationException("No data has been logged in "
                                            "current session.")
            ...
            try:
                for f in self._moku._fs_list(mp):
                    if str(f[0]).startswith(target):
                        # Don't overwrite existing files of the name name.
                        # This would be nicer if we could pass receive_file a
                        # local filename to save to, but until that change is
                        # made, just move the clashing file out of the way.
                ...
                log.debug(
                    "Attempted to list files on unmounted device '%s'" % mp)
    ...
    def get_timestep(self):
        ...
        This returns the inverse figure to `get_samplerate`. This form is more
        useful for constructing time axes to support, for example,
        `get_stream_data`.
