...
class FRAData(_frame_instrument.InstrumentData):
    """
    Object representing a frame of dual-channel (amplitude and phase) vs
    frequency response data.

    This is the native output format of the :any:`FrequencyResponseAnalyzer`
    instrument.

    This object should not be instantiated directly, but will be returned by
    a call to :any:`get_data
    <pymoku.instruments.FrequencyResponseAnalyzer.get_data>` on the
    associated :any:`FrequencyResponseAnalyzer` instrument.
    ...
    def __init__(self, instrument, scales):
        ...
        # Obtain all data scaling factors relevant to current NetAn
        # configuration
        self.scales = scales

    def __json__(self):
        # Annoying this doesn't recursively-descend, I thought it did.
        # Manually serialise the children for now
        return {'ch1': self.ch1.__json__(),
                'ch2': self.ch2.__json__(),
    ...
    def process_complete(self):
        ...
        if self._stateid not in self.scales:
            # log.debug("Can't render FRAData frame, haven't saved calibration
            # data for state %d", self._stateid)
            self.complete = False
            return
        ...
        try:
            ...
            self.ch1 = _FRAChannelData(self.ch1_bits,
                                       scales['gain_correction'],
                                       scales['g1'],
                                       scales['sweep_amplitude_ch1'])
            ...
            self.ch2 = _FRAChannelData(self.ch2_bits,
                                       scales['gain_correction'],
                                       scales['g2'],
                                       scales['sweep_amplitude_ch2'])
