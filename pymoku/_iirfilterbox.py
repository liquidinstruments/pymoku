...
class IIRFilterBox(_CoreOscilloscope):
    ...
    def set_filter(self, ch, sample_rate, filter_coefficients):
        """
        Set SOS filter sample rate and filter coefficients. This also enables
        the input and outputs of the specified Moku:Lab channel.

        :type ch: int; {1,2}
        :param ch: target channel

        :type sample_rate: string; {'high','low'}
        :param sample_rate: filter sample rate where 'high' ~ 15.625 MHz and
            'low' ~ 122.070 kHz.

        :type filter_coefficients: array;
        :param filter_coefficients: array containing SOS filter coefficients.
            Format is described in class documentation above.
        """
    ...
    def set_monitor(self, monitor_ch, source):
        """
        Configures the specified monitor channel to view the desired IIR
        Filter Box signal.
        ...
        internal filterbox monitoring points. Signals larger than 12-bits
    ...
    def set_trigger(self, source, edge, level, minwidth=None, maxwidth=None,
        ...
        :param source: Trigger Source. May be either an input or monitor
                        channel (as set by
                :py:meth:`~pymoku.instruments.IIRFilterBox.set_monitor`),
                or external. External refers to the back-panel connector of
                the same name, allowing triggering from an
