...
class MokuInstrument(object):
    """Superclass for all Instruments that may be attached to a :any:`Moku`
    object.

    Should never be instantiated directly; instead, instantiate the subclass
    of the instrument you wish to run (e.g. :any:`Oscilloscope`,
    :any:`WaveformGenerator`)
    """

    def __init__(self):
        """ Must be called as the first line from any child
        implementations. """
        self._accessor_dict = {}
        self._moku = None
    ...
    def set_defaults(self):
        ...
        # These may be called again in the instrument's implementation to
        # overwrite this, however they must be called at least once to load
    ...
    def commit(self):
        ...
        .. note::
            ...
            If the `autocommit` feature has been turned off, this function
            can be used to manually apply any instrument settings to the Moku
            device. These instrument settings are those configured by calling
            all *set_* and *gen_* type functions. Manually calling this
            function allows you to atomically apply many instrument settings
            at once.
        """
        # We wrap up the implementation of the actual commit so we dont
        # expose the update_state parameter to normal users
    ...
    def _sync_registers(self):
        ...
        This should never have to be called explicitly, however in advanced
        operation where the Moku state is being updated outside of pymoku,
    ...
    def _on_reg_sync(self):
        # This function acts when a Moku's register state is
        # synchronised to the local Moku instance. It is useful to update
        # local variables and perform sanity checks on the device state
        # before it is deemed 'useable'.
        ...
        if not any(reg_data):
            log.warning(
                "Moku seems to be unconfigured - setting instrument defaults.")
            self.set_defaults()
            self.commit()
    ...
    def _dump_remote_regs(self):
        ...
        This should never have to be called explictly, however in advanced
        operation where the Moku state is being updated outside of pymoku,
        this gives the user access to the register values directly.

        Unlike :any:`sync_registers`, no local state is updated to reflect
        these register values and they are not made available through
    ...
    def _set_running(self, state):
        ...
        This is used to clean up helper threads and sockets. Should never be
        called explicitly.
        """
        self._running = state
    ...
    def _set_instrument_active(self, active):
        ...
        This should never have to be called explicitly, as the instrument is
        correctly reset when it is attached and detached. In advanced
        operation, this can be used to force the instrument in to its
        initial state without a redeploy.
        """
    ...
    def _dac_gains(self):
        ...
        # For now, assume a fixed 48 degrees C board temperature. In future,
        # should read temperature registers
        g1 += gt1 * 48.0
        g2 += gt2 * 48.0

        # The sense of these parameters as used in pymoku is inverted from the
        # storage on the Moku
        g1 = 1 / g1
        g2 = 1 / g2
    ...
    def _adc_gains(self):
        ...
        # The sense of these parameters as used in pymoku is inverted from
        # the storage on the Moku
        g1 = 1 / g1
        g2 = 1 / g2
    ...
    def _load_feature(self, index):
        # For now we don't support switching clock modes during a partial
        # deploy
        self._moku._deploy(
            use_external=self._moku.external_reference, partial_index=index)
...
_instr_reg_handlers = {
    ...
    'hwver':
        (REG_ID2,
            None,
            from_reg_unsigned(24, 0)),
    'hwserial':
        (REG_ID2,
            None,
            from_reg_unsigned(0, 24)),
    'pause':
        (REG_PAUSE,
            to_reg_bool(0),
            from_reg_bool(0)),
    'frame_length':
        (REG_OUTLEN,
            to_reg_unsigned(0, 12),
            from_reg_unsigned(0, 12)),
    'keep_last':
        (REG_OUTLEN,
            to_reg_bool(28),
            from_reg_bool(28)),
    ...
    'x_mode':
        (REG_OUTLEN,
            to_reg_unsigned(29, 2, allow_set=[ROLL, SWEEP, FULL_FRAME]),
            from_reg_unsigned(29, 2)),

    'render_mode':
        (REG_FILT,
            to_reg_unsigned(0, 2, allow_set=[RDR_CUBIC,
                                             RDR_MINMAX,
                                             RDR_DECI,
                                             RDR_DDS]),
    ...
    'render_dds':
        (REG_SCALE,
            to_reg_unsigned(0, 16, xform=lambda obj, x: x - 1),
            from_reg_unsigned(0, 16, xform=lambda obj, x: x + 1)),

    'render_dds_alt':
        (REG_SCALE,
            to_reg_unsigned(16, 16, xform=lambda obj, x: x - 1),
    ...
    'temp_dac':
        (REG_TEMP_DAC,
            None,
            from_reg_signed(0, 12, xform=lambda obj, f: f * 0.0625)),
    'temp_adc':
        (REG_AINCTL,
            None,
            from_reg_signed(20, 12, xform=lambda obj, f: f * 0.0625)),
}
