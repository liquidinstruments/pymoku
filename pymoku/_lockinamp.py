...
class LockInAmp(PIDController, _CoreOscilloscope):
    ...
    def set_input_gain(self, gain=0):
        ...
        """
        _utils.check_parameter_valid('set', gain, allowed=[-20, 0, 24, 48],
                                     desc="main input gain", units="dB")
        front_end_setting = self.get_frontend(1)

        if gain == 0:
            self.input_gain_select = 0
            self.set_frontend(1, fiftyr=front_end_setting[0],
                              atten=False, ac=front_end_setting[2])
        elif gain == 24:
            ...
            self.input_gain_select = 0
            self.set_frontend(1, fiftyr=front_end_setting[0],
                              atten=True, ac=front_end_setting[2])
        else:
    ...
    def set_outputs(self, main, aux, main_offset=0.0, aux_offset=0.0):
        """
        Configures the main (Channel 1) and auxillary (Channel 2) output
        ...
        self.aux_select = 1 if aux in (_LIA_SIGNALS) \
            else (2 if aux == 'demod' else 0)
        # Defaults to local oscillator i.e. 'sine'

        # PID/Gain stage selects are updated on commit

    def _update_pid_gain_selects(self):
        # Update the PID/Gain signal inputs / channel select ouputs to match
        # the set main/aux source signals

        def _signal_select(sig):
            return 0 if not(sig in _LIA_SIGNALS) \
                else [i for i, x in enumerate(_LIA_SIGNALS) if x == sig][0]
    ...
    def set_pid_by_frequency(self, lia_ch, kp=1, i_xover=None, d_xover=None,
        ...
        """
        Set which lock-in channel the PID is on and configure it using
    ...
    def set_pid_by_gain(self, lia_ch, g, kp=1.0, ki=0, kd=0, si=None,
        ...
        """
        Set which lock-in channel the PID is on and configure it using gain
        parameters.
        ...
        # commit
        # This ensures all dependent register values are updated at the same
        # time, and the correct DAC scaling is used.
        self._pid_channel = lia_ch
        self._pid_gains = {'g': g,
                           'kp': kp,
                           'ki': ki,
    ...
    def set_gain(self, lia_ch, g):
        ...
        :param g: Gain
        """
        _utils.check_parameter_valid('set', lia_ch, allowed=['main', 'aux'],
                                     desc="lock-in channel")
        _utils.check_parameter_valid('range', g, allowed=[0, 2**16 - 1],
                                     desc="gain")

        # Store selected PID channel locally. Signal select regs are updated
        # on commit.
    ...
    def set_demodulation(self, mode, frequency=1e6, phase=0,
        ...
        if mode == 'internal':
            ...
            # Should not happen
            raise ValueOutOfRangeException('Demodulation mode must be one '
    ...
    def set_lo_output(self, amplitude, frequency, phase):
        ...
        :param phase: Phase
        """
        _utils.check_parameter_valid('range', amplitude, allowed=[0, 2.0],
        ...
        _utils.check_parameter_valid('range', phase, allowed=[0, 360],
                                     desc="local oscillator phase",
                                     units="degrees")

        # The sine amplitude register also scales the LIA signal outputs
        # (eek!), so it must only be updated if the auxillary output is set
        # to a non-filtered signal.
...
_lia_reg_hdl = {
    ...
    'ch1_pid1_int_i_gain':
        (REG_LIA_INT_IGAIN2,
            to_reg_signed(0, 25, xform=lambda obj, x: x),
