...
from pymoku._instrument import ADC_SMP_RATE, needs_commit, deprecated
from pymoku._instrument import to_reg_bool, from_reg_bool
from pymoku._instrument import to_reg_signed, from_reg_signed
from pymoku._instrument import to_reg_unsigned, from_reg_unsigned
...
from pymoku._utils import check_parameter_valid as valid
from pymoku._utils import str_to_val
...
class ArbitraryWaveGen(_CoreOscilloscope):
    ...
    def write_lut(self, ch, data, mode=None):
        ...
        valid('set', mode, [125, 250, 500, 1000], desc='output sample rate',
              units="MSmps", allow_none=True)
        ...
        self._moku._send_file_bytes('j', '', byte_data,
    ...
    def gen_waveform(self, ch, period, amplitude, phase=0, offset=0,
        ...
        *dead_voltage*.  This allows the user to, for example, generate
        ...
        valid('range', amplitude, [0.0, 2.0], desc='peak to peak amplitude',
              units='volts')
        valid('bool', interpolation, desc='linear interpolation')
        valid('range', dead_time, [0.0, 2e18], desc='signal dead time',
              units='cycles')
        valid('range', dead_voltage, [-2.0, 2.0], desc='dead value',
              units='volts')
    ...
    def _update_dependent_regs(self, scales):
        ...
        self._trigger1.level = int(round(
            self.trig_level1 / self._signal_source_volts_per_bit(
                self.trig_source1, scales)))
        self._trigger2.level = int(round(
            self.trig_level2 / self._signal_source_volts_per_bit(
                self.trig_source2, scales)))
    ...
    @deprecated(category='method',
                message="'gen_off' has been deprecated,"
                        " use 'enable_output' instead.")
    ...
    def _signal_source_volts_per_bit(self, source, scales, trigger=False):
        ...
        TODO: complete this function when osc functionality added to awg,
        stubbed for now.
        """
...
_arb_reg_handlers = {
    ...
    'mode1':
        (REG_ARB_SETTINGS1,
            to_reg_unsigned(2, 2, allow_set=[_ARB_MODE_125, _ARB_MODE_250,
                                             _ARB_MODE_500, _ARB_MODE_1000]),
    ...
    'trig_source1':
        (REG_ARB_SETTINGS1,
            to_reg_unsigned(5, 5, allow_set=[_ARB_TRIG_SRC_CH1,
                                             _ARB_TRIG_SRC_CH2,
                                             _ARB_TRIG_SRC_EXT]),
    ...
    'amplitude1':
        (REG_ARB_AMPLITUDE1,
            ...
            from_reg_signed(0, 18, xform=lambda obj,
                            r: r * obj._dac_gains()[0])),
    'offset1':
        (REG_ARB_OFFSET1,
            to_reg_signed(0, 16, xform=lambda obj, r: r / obj._dac_gains()[0]),
            from_reg_signed(0, 16, xform=lambda obj,
                            r: r * obj._dac_gains()[0])),
    'enable2':
        (REG_ARB_SETTINGS2,
            to_reg_bool(0),
    ...
    'amplitude2':
        (REG_ARB_AMPLITUDE2,
            to_reg_signed(0, 18, xform=lambda obj, r: r / obj._dac_gains()[1]),
            from_reg_signed(0, 18, xform=lambda obj,
                            r: r * obj._dac_gains()[1])),
    'offset2':
        (REG_ARB_OFFSET2,
            to_reg_signed(0, 16, xform=lambda obj, r: r / obj._dac_gains()[1]),
            from_reg_signed(0, 16, xform=lambda obj,
                            r: r * obj._dac_gains()[1]))
}
